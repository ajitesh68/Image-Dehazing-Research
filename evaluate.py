    ...
    with torch.no_grad():
        for batch_idx, (hazy, clean) in enumerate(test_loader):
            ...
            if save_results and batch_idx == 0:
                save_comparison(hazy[:4], prediction[:4], clean[:4], save_path=f'{output_dir}/test_results.png')
    results = {'psnr': psnr_meter.avg, 'ssim': ssim_meter.avg}
    print(f"\n{'=' * 40}")
    print(f'  TEST RESULTS (Final Score!)')
...
def main():
    parser = argparse.ArgumentParser(description='Dehazing Model Evaluate Karo')
    parser.add_argument('--config', type=str, default='configs/default.yaml')
    parser.add_argument('--checkpoint', type=str, default='experiments/best_model.pth', help='Saved model checkpoint ka path')
    ...
    print(f"(Val loss: {checkpoint['val_loss']:.4f}, Val PSNR: {checkpoint['val_psnr']:.2f})")
