...
def get_device(config):
    ...
    if device_choice == 'tpu':
        try:
            ...
            print('⚠️ torch_xla not installed. Install with: pip install torch_xla')
    ...
    print('   TIP: Google Colab mein FREE GPU/TPU milta hai!')
    return device
...
def train_one_epoch(model, train_loader, criterion, optimizer, device, epoch):
    ...
    for batch_idx, (hazy, clean) in enumerate(train_loader):
        ...
        if (batch_idx + 1) % max(1, len(train_loader) // 5) == 0:
            print(f'  Epoch [{epoch}] Batch [{batch_idx + 1}/{len(train_loader)}] Loss: {loss.item():.4f} | PSNR: {psnr:.2f} dB')
...
def main():
    ...
    print('\n🧠 Model ban raha hai...')
    model = UNet(in_channels=config['model']['in_channels'], out_channels=config['model']['out_channels'], features=config['model']['features'], use_batch_norm=config['model']['use_batch_norm'], dropout_rate=config['model']['dropout_rate'])
    model = model.to(device)
    total_params = sum((p.numel() for p in model.parameters()))
    print(f'Model parameters: {total_params:,}')
    criterion = get_loss_function(config)
    optimizer = optim.Adam(model.parameters(), lr=config['training']['learning_rate'])
    scheduler = ReduceLROnPlateau(optimizer, mode='min', patience=config['training']['scheduler']['patience'], factor=config['training']['scheduler']['factor'])
    ...
    for epoch in range(1, epochs + 1):
        ...
        if val_metrics['loss'] < best_val_loss:
            ...
            torch.save({'epoch': epoch, 'model_state_dict': model.state_dict(), 'optimizer_state_dict': optimizer.state_dict(), 'val_loss': val_metrics['loss'], 'val_psnr': val_metrics['psnr'], 'val_ssim': val_metrics['ssim']}, save_path)
